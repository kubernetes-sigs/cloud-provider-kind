...
// envoyConfig returns an envoy config generated from config data
func envoyConfig(data *configData) (config string, err error) {
...
func (c *Controller) createGateway(clusterName string, gateway *gatewayv1.Gateway, dynamicFilesystemConfig string) error {
	...
	args := []string{
		...
		"--label", fmt.Sprintf("%s=%s", constants.GatewayNameLabelKey, gatewaySimpleName(clusterName, gateway)),
